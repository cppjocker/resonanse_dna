...
input_file = 'chr15.fa'
heatmap_file = "heatmap_15.csv"
chrom_start_file = "chrom15_start"
chrom_name = "Chrom 15"

do_insert = False
do_insert_fract = False
do_randomize = True

alg_mode = 0
code_mode = 1 # 0 is 0->1 code, 1 is -1->1 code
stage = 1 # 0 - processing, 1 - visualisation
...
def make_periodogramm(seq_np, size2, idx_gaps):
...
def make_corr_func_fast(seq_np, size2):
    ...
    for i in range(0, len_corr_func):
        ...
        vect = np.vectorize(make_correlation)
...
def make_corr_func(seq_np, size2):
    len_corr_func = int(len(seq_np) / 2)
...
def make_corr_func_cpp(seq_np, size2):
...
def insert_random2(sequence, repeats):
    insert_place1 = 38000000
    rand_len1 = 7

    sequence = insert_random_next(sequence, repeats, insert_place1, rand_len1)

    return sequence

def insert_random(sequence, repeats):
    ...
    sequence = insert_random_next(sequence, repeats, insert_place1, rand_len1)
    sequence = insert_random_next(sequence, repeats, insert_place2, rand_len2)
    sequence = insert_random_next(sequence, repeats, insert_place3, rand_len3)
...
def make_heatmap():
    ...
    if chunk_start != 0 and chunk_end != 0:
        i_s = max(i_s, chunk_start)
        i_e = min(i_e, chunk_end)
    ...
    if do_insert_fract:
        sequence = insert_random2(sequence, 60000)

    if do_insert:
        sequence = insert_random(sequence, 6000)
    ...
    for k in range(0, bins):
        ...
        if alg_mode == 0:
            psd = make_periodogramm(seq_np, size2, idx_gaps)
        elif alg_mode == 1:
            psd = make_corr_func(seq_np, size2)
        elif alg_mode == 2:
            psd = make_corr_func_cpp(seq_np, size2)
    ...
    np.savetxt(chrom_start_file, np.array([i_s]) )

    np.savetxt(heatmap_file, heatmap_mat, delimiter=";")
